...
- **`CrowdController`** owns a `NativeArray<UnitData>` (position, formation slot, animation phase, alive flag). Formation slots use a **phyllotaxis (sunflower) spiral** so the crowd grows and shrinks visually from the center without reshuffling — adding 200 units on a `×` gate never reorders existing ones.
